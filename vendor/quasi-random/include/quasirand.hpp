 ...
 *
 * The dimension of the generator can be chosen at runtime if the Dim argument
 * is set to DYNAMIC.
 */
...
template <std::floating_point RealType> class QuasiRandom<0, RealType>;

/* DYNAMIC GENERATOR */

/* Specialization of the generator with a dynamic number of dimensions. */
template <std::floating_point RealType> class QuasiRandom<DYNAMIC, RealType> {
public:
...
constexpr QuasiRandom<Dim, RealType>::QuasiRandom() noexcept : seed_(0.5) {
...
template <std::size_t Dim, std::floating_point RealType>
constexpr QuasiRandom<Dim, RealType>::QuasiRandom(RealType seed) : seed_(seed) {
...
template <std::size_t Dim, std::floating_point RealType>
constexpr auto
QuasiRandom<Dim, RealType>::operator()() noexcept -> result_type {
...
template <std::size_t Dim, std::floating_point RealType>
constexpr auto QuasiRandom<Dim, RealType>::operator()(size_type n
...
template <std::floating_point RealType>
constexpr auto QuasiRandom<DYNAMIC, RealType>::operator()(size_type n
) const -> result_type {
...
template <std::floating_point RealType>
constexpr auto
QuasiRandom<DYNAMIC, RealType>::dim() const noexcept -> size_type {
  return dim_;
}
